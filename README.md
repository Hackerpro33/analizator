...
- Для демо-режима доступен сидер `cd backend && python -m app.seed_security_events`. Он синтезирует 500 событий по цепочке recon → auth brute-force → lateral movement → exfiltration, включая GeoIP и MITRE ATT&CK фазы. Сид выполняется идемпотентно и совместим как с SQLite, так и с PostgreSQL.
